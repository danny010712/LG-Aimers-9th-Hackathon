...
- **trackman**(1,793,078행, 유일하게 안 쓰는 데이터): 투수 단위 매핑은 복원 성공(등판 지문).
  ⚠️ **"정보 중복이라 재시도 금지"로 적혀 있었으나 2026-08-24 재검토 결과 부정확하다**(08 §6-G).
  실제로 잰 것은 **매핑 신뢰도가 낮은 상태의 이득 0**이다(필터 없음 ~702 = 매핑 틀린 54%가
  노이즈 과적합 / `sim≥0.8` 780.5~782.6 = base 780.0과 동급).
  그리고 **넣었던 23개 피처가 무엇인지 기록에 없다.** 문헌상 command의 1차 예측자는
  **릴리스 포인트 일관성**(`rel_side` 표준편차)인데 평균값만 넣었다면 안 재본 것이다.
  → **조건부 재시도 가능.** `sim≥0.8` 필터 필수, 분산·일관성 피처 위주로.
...
3. ⭐ **trackman 재검토** (08 §6-G). "정보 중복"이라 적혀 있으나 실제로 잰 건
   "매핑 신뢰도 낮은 상태의 이득 0"이고, **넣었던 23피처가 무엇인지 기록에 없다.**
   문헌은 command의 1차 예측자로 **릴리스 포인트 일관성**(`rel_side` 표준편차)을 꼽는다.
   평균값만 넣었다면 안 재본 것이다. `scratchpad/pitcher_map.csv`·`pitcher_profile_*.csv`가
   남아 있으면 **재학습 없이 프로브 가능.** 유일하게 안 쓰는 데이터(1,793,078행)다.
...
- 남은 미개척 정보원은 **trackman 1,793,078행 하나뿐**이다.
