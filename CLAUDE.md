...
test/       train_local.py → train_offset.py → build_shift.py → verify_submit.py  (이 순서)
            recover_labels.py · recovered_labels.csv.gz  (숨은 라벨 8종)
            map_trackman.py + pitcher_map_seq.csv (게임시퀀스 정렬, 매핑 99%) · build_tm_profile.py
            make_valpred.py  ⚠️ 이제 train_local.py가 검증 예측을 직접 저장한다. 이 파일은
              옛 run의 캐시를 다시 만들 때만 쓴다(같은 모델을 두 번 학습하므로 평소엔 불필요)
            verify_submit.py  ← 제출 전 필수. import↔zip 대조까지 한다
            probe_offset_forms.py(연도별 창 12모델) + _preds.csv.gz + _score.py(전이 매트릭스)
            probe_pitcher_headroom.py  ← 투수 고정 피처의 천장 측정(§7-1). 13 BSS
            ...
            artifacts/auxpred/*.npy       ← 003 피처(57열) 검증예측. 009~012 계보 + mr/wayoff
            artifacts/auxpred_ins/*.npy   ← 시즌내 분해 피처 검증예측. 013 이후 계보
...
- **trackman**(1,793,078행): 🔴 **축 종료 확정 — 2026-08-24 재검토 완료 (09 §2-F).**
  재검토 사유 2개를 둘 다 해소했고 **결과는 부정**이다.
  ① **매핑 46% → 99%.** 등판 지문 대신 **게임 시퀀스 정렬**(`row_id`가 시간순인 점 이용:
     첫 40투구의 이닝·초말·볼·스트라이크·아웃 시퀀스 = 사실상 식별자) →
     `test/map_trackman.py` → `pitcher_map_seq.csv`. 게임 4,094/4,868 유일매칭·**중복 0**,
     투수 **716명**(792명의 90.4%)·순도 **.9877**·충돌 0, **train 투구 커버리지 99.8%**.
  ② **"평균만 넣었을 것"도 해소.** 문헌이 지목한 `rel_side` **표준편차**(구종 내 sd 가중평균,
     레퍼토리 혼합 제거)로 재검정 → **실제 2024 성공률과 상관 −0.037** (`asof`는 **+0.468**).
     `asof` 5분위 내 증분 **0**(열 평균 .4871~.4924, 단조 아님).
  → **"정보 중복"은 매핑 신뢰도의 부작용이 아니라 실재였다. 재시도 금지.**
  추정 이유: MLB 문헌의 결과변수는 볼넷·홈런인데 **우리 타깃은 주최측 운영 라벨**
  (한복판/크게벗어남/포수요구반대)이라 물리적 정확도와 다른 것을 잰다.
  🔍 **독립 감사로 재확인(같은 날 밤).** 문서가 든 근거엔 구멍이 둘 있었다 —
     발표값이 **원**상관이었고(부분상관은 −0.088~−0.116으로 문헌 부호와 맞다),
     **4열만** 쟀다. 17열(물리 8 × sd·평균 + 구종믹스)로 넓혀 out-of-year·레벨제거로 재니
     **+120.7 BSS**가 나왔다. 그러나 그 베이스는 `sm0` 1열이었다 —
     **실모델(013) 잔차로 재면 −10.3.** §5의 "프로브 base가 낮으면 트리가 이미 잡던 몫까지
     이득으로 센다"가 그대로 재현됐다. **결론 유지, 근거는 더 강해졌다.**
  ✅ 매핑은 영구 자산 — 다시 볼 일이 생기면 46%가 아니라 99%에서 시작한다.
...
| 866.3 | 1016.39 | 018 = 013 +ins 5열 확장 · **raw**(offset·shift 없음, 실수 제출) |
...
🔴 **015가 국소 최적이다. 넓히는 두 방향이 둘 다 졌다** (2026-08-24 저녁, 08 §5-10).
- **017** 보조모델도 61열 재학습 → **−4.69.** 계수 `b` −0.0990→−0.0835(16%↓) = 보조가
  성공모델과 **같은 입력**을 보니 예측이 닮아 밀 여지가 줄었다. → **보조는 003 피처 57열 고정.**
- **020** 주모델에 ins 5열 추가(reverse/middle/ball/strike/타자middle) → **−5.14.**
  로컬은 +7.7이었으나 짝지은 시드차 0.8σ·부호 2/3·시드 편차 3.5배 증가로 **이미 기준 미달**이었다.
  5열이 서로 겹친다("이 선수의 올해 환경") = **정보는 안 늘고 용량만 늘었다.**
- ⚠️ **선형 프로브를 채택 도구로 쓰지 말 것.** 프로브 +73.7 → 실모델 +7.7(0.10배).
  `success` 때는 0.58배였다. 프로브 base가 낮아 **트리가 이미 잡던 몫까지 이득으로 센다.**

🔥 **offset·shift는 베이스가 강해지면 기여가 줄어든다** — 실수로 낸 018 raw가 드러냈다.
```
003 raw  로컬 783.3  LB  881.73        +offset+shift → ~992.5 = +110.7
018 raw  로컬 866.3  LB 1016.39        +offset+shift → 1046.59 = +30.2   (73% 감소)
```
raw만으로 팀원 최고(1012.70)를 넘었다. 세 축은 독립이 아니라 **부분 대체재**다.
⚠️ §5-10 앞부분의 "겹치지 않는다"는 **로컬 2024** 측정이다. 2025는 미관측이라 세 축이 같은
구멍을 두고 경쟁한다. 🔴 **로컬에서 잰 독립성이 LB에서 유지된다고 가정하지 말 것.**

**전이율**: offset +26.6→**+63.7**(2.40) · 시드 +3.8→+5.6(1.47) · **시즌내 +75.3→+59.2(0.79)** · **시즌내 raw +83.0→+134.7(1.62)** · cond +12.5→**−5.4** · depth8 +7.6→**−34.2**
...
2. ⭐ **시드 3 → 7** (+5.6 기지, ~56분). 언제든 넣을 수 있는 확실한 이득. **015 기반으로.**
3. ❌ ~~trackman 재검토~~ — **2026-08-24 완료, 부정**(§2 trackman 항목 / 09 §2-F).
   매핑 99% + `rel_side` sd로 재검정했으나 실제 성공률과 상관 **−0.037**, `asof` 통제 시 증분 **0**.
   학습은 돌리지 않았다(증분이 0으로 측정되면 학습이 못 뒤집는다).
4. ⭐ **보조모델에 자기 타깃의 시즌내 열**(`ins_pitcher_middle_rate` 등). 016이 준 것과
   **종류가 다른 정보**다(016은 성공률만 줬다). 단 017이 이미 졌고 로컬 판정 불가.
5. **팀원 트랙**(08 §5-11): p_matchup(+3.3)만 🟢. league baseline은 시즌내와 **정면 중복
   위험**(외삽 vs 관측) + 재학습 2h. aux baseline은 017이 이미 반증.
6. **로컬 관측점 확보**(LB 공개 모델의 로컬 점수 측정, 제출권 0).
...
**시즌내 분해 확장 2종 기각(2026-08-24)**: 보조모델을 주모델 피처로 재학습(**−4.69**) · 주모델에 ins 5열 추가(**−5.14**). 015(success 1종)가 국소 최적. → CLAUDE.md §5, 08 §5-10.

## 7-1. 🔴 판별식 — 새 축을 볼 때 이것부터

**"정보가 느는가, 표현이 느는가."**
소진 목록은 전부 **기존 컬럼 재조합 = 정보 증가 0**이라 전멸했다. 시즌내 분해(+75.3)만
모델이 도달 불가능한 값(`n₀`,`s₀` = 학습에서 만든 선수별 기준점)을 넣어서 이겼다.
- 표현력을 늘리는 방향(피처 조합·깊이·앙상블 축) = **재시도 금지.**
- **정보가 늘어도 서로 겹치면 진다** — 020의 ins 5열이 그랬다(전부 "올해 환경"을 인코딩).
- ~~남은 미개척 정보원은 trackman 하나뿐~~ → **2026-08-24 소진**(09 §2-F).
  **정보원 축은 전부 닫혔다.** 남은 것은 정보 추가가 아니라 기존 정보의 정밀도·분산감소뿐.

🔥 **판별식에 두 번째 질문이 붙는다: "행마다 변하는가, 선수마다 상수인가."**
(2026-08-24 밤 감사, `test/probe_pitcher_headroom.py` / 09 §2-F)
투수별 실제 성공률 잔차에서 이항 표본노이즈를 빼 **남은 참신호**를 재면:
```
통제 없음                    758 BSS
시즌시작 통산(스무딩) 1열      676 BSS
**013 실모델**               13 BSS   ← 이항노이즈에 묻힘
```
→ 🔴 **투수마다 상수인 피처는 종류를 불문하고 천장이 ~13 BSS다.** 축 하나가 아니라
**클래스 하나**가 닫힌다 — 물리 프로필·스카우팅·구종 레퍼토리·경력 요약이 전부 여기다.
시즌내 분해가 이 클래스가 **아니었던** 이유도 이것 — 그건 1구마다 값이 변한다.
