...
test/       train_local.py → train_offset.py → build_shift.py → verify_submit.py  (이 순서)
            ...
            build_platoon.py  ← 투수 좌우편차 offset (015 위에 얹는다, 학습 없음)
            common/league_rate.py  ← 팀원 코드. R/F 분리 외삽 (빌드 전용, zip 미포함)
            probe_platoon_offset.py  ← 독립 베이스로 재확인
...
| — | **1057.00** | **021 = 015 +투수 좌우편차 offset** ← 최고. 로컬 +20.5 → LB **+5.27**(전이 0.26) |
...
**전이율**: offset +26.6→**+63.7**(2.40) · 시드 +3.8→+5.6(1.47) · **시즌내 +75.3→+59.2(0.79)** · **시즌내 raw +83.0→+134.7(1.62)** · cond +12.5→**−5.4** · depth8 +7.6→**−34.2** · **좌우편차 +20.5→+5.3(0.26)**
→ **out-of-year 델타는 부호가 유지되고, fold 내부 델타는 부호가 뒤집힌다.**
→ 🔴 **"증폭된다"는 틀렸다** (2026-08-24 교정). 관측 2개로 세운 규칙을 세 번째(0.79)가 깼다.
  **배율은 예측 불가.** 🔴 **2026-08-25 재교정**: 021(좌우편차)이 **0.26배**로 하한을 다시 깼다.
  관측 배율 **0.26 / 0.79 / 1.47 / 1.62 / 2.40** — 9배 폭이다.
  → **out-of-year 델타로 예측 가능한 것은 부호뿐이다. 크기는 아니다.** 밴드를 적지 말 것.
  🔥 **단, 배율에 클래스가 있다** (08 §5-13). 흩어진 게 아니라 두 무리다:
  ```
  드리프트/외삽을 고치는 축     offset 2.40 · 시즌내raw 1.62 · 시드 1.47 · 시즌내 0.79 · shift ~1.0
  판별력만 올리는 축           좌우편차 0.26 · cond 음수 · depth8 음수
  ```
  → **"이게 2025의 미관측성을 건드리는가?"** 아니면 로컬 델타의 **0.3배**를 기대값으로 잡을 것.
  ⚠️ 021은 로컬에서 "베이스를 003→018로 바꿔도 이득이 안 준다"를 확인하고 낸 것인데도 0.26이었다.
  **베이스 4개에서 잰 비대체성조차 LB로 전이되지 않았다** — §5-10의 교훈이 더 강한 형태로 재발했다.
...
2. ✔️ **투수 좌우편차 offset (run 021)** — **채택됨, LB 1057.00 (+5.27).**
   `cond`(004 −5.4)와 **같은 정보 다른 전달**(트리 피처 → 잔차 offset, 용량 증가 0)이
   부호를 뒤집었다. 단 **로컬 +20.5 → LB +5.27 = 전이 0.26배**로 역대 최저. 08 §5-12.
3. ⭐ **시드 3 → 7** (+5.6 기지). run 022~025 체인으로 진행 중.
3-1. ⭐ **`EST_2025` 성분별 외삽** (run 026, ~+5). R은 전체구간·F는 `min_season=2023`.
   `common/league_rate.py`(팀원 코드) 재사용. 재학습 없음. 08 §5-14.
3-2. ⭐⭐ **mr 보조모델에 자기 타깃 시즌내 분해** (run 027). `ins_pitcher_{middle,reverse}_rate`.
   **wayoff는 불가** — `asof_*_wayoff_rate`가 데이터에 없다. 그래도 `b`가 `c`의 13배라
   개선 여지가 애초에 mr뿐이다. 017과 **방향이 반대**(주모델 피처를 주는 게 아니라
   주모델에 **없는** 열을 준다). 남은 아이디어 중 상승 여지가 실재하는 유일한 것.
   ⚠️ 순서: 026을 **먼저** 내고 그 다음 027. `ANCHOR_SPECS`를 늘리면 anchor.csv 스키마가
   바뀌어 옛 스키마 위에 얹으면 `KeyError`로 죽는다(2026-08-25에 실제로 겪음).
4. ❌ ~~trackman 재검토~~ — **2026-08-24 완료, 부정**(§2 trackman 항목 / 09 §2-F).
   매핑 99% + `rel_side` sd로 재검정했으나 실제 성공률과 상관 **−0.037**, `asof` 통제 시 증분 **0**.
   학습은 돌리지 않았다(증분이 0으로 측정되면 학습이 못 뒤집는다).
5. ⭐ **보조모델에 자기 타깃의 시즌내 열**(`ins_pitcher_middle_rate` 등). 016이 준 것과
   **종류가 다른 정보**다(016은 성공률만 줬다). 단 017이 이미 졌고 로컬 판정 불가.
6. **팀원 트랙**(08 §5-11): p_matchup(+3.3)만 🟢. league baseline은 시즌내와 **정면 중복
   위험**(외삽 vs 관측) + 재학습 2h. aux baseline은 017이 이미 반증.
7. **로컬 관측점 확보**(LB 공개 모델의 로컬 점수 측정, 제출권 0).
...
**투수×상황 offset 클래스 전멸(2026-08-25)**: 투수×카운트 0 · 투수×이닝 0 · 투수×2스트라이크 0 · **투수×주자유무 0.46** (4전 4패). 지속성 corr 좌우 **0.412** vs 주자 0.106 · 월 0.049 → 전이가능 ≤6.8 BSS. 표본 조건은 동일(분할 52:48, 셀 중앙값 344 vs 339)이므로 데이터 부족이 아니라 **신호 부재**. 살아남은 것은 좌우 하나뿐 — **그것만 신체 구조**(팔 각도×타석 위치)라 해마다 이어진다. → 09 §2-G.
🔴 **08 §5-12 축 분해표의 "남은 BSS(여지)" 열을 채택 근거로 쓰지 말 것.** 여지는 이항노이즈만 빼고 **재현 안 되는 진짜 변동**은 못 거른다 — 죽은 것이 확정된 투수×카운트가 재측정에서 **75.5**로 나온다. offset 표 후보는 **지속성**(≤T−1 표 vs T 표 상관)으로 판정한다. 021이 채택근거 ①을 그것으로 둔 게 정답이었다. → 09 §0-5.
**2026-08-25 추가 기각**: Saerens EM 등 **prior-shift 보정 계열**(test 미라벨 분포 사용 = 규정 위반) ·
**이질적 드리프트 보정**(2024 전 세그먼트 0, 231~1628은 죽은 fold 2023) · **포수 프레이밍**(데이터에 포수 컬럼 없음) ·
🔴 **타자 축 전체**(`asof_batter_*`가 3열뿐 — 가능한 조합 `success`·`middle` 둘 다 이미 시험됨. 잔차 여지 0).
...
✅ **그 예측이 곧바로 맞았다** — 잔차를 축별로 분해하니 **투수×좌우 126 BSS**가 남아 있었고
(투수 단독은 22, 타자 단독·타자×좌우는 **0**), 이게 run 021이 됐다. 좌우편차는 상수가 아니라
**상대 타자에 따라 행마다 바뀐다** = 위 천장의 바깥이다. 08 §5-12.

🔥 **세 번째 질문이 붙는다: "해마다 이어지는가."** (2026-08-25, 09 §2-G)
행마다 변하기만 해선 부족하다 — **작년 표가 올해에도 맞아야** offset으로 쓸 수 있다.
투수×주자·투수×월은 첫 질문(행마다 변함)은 통과하고 여기서 죽었다(corr 0.106 · 0.049 vs 좌우 0.412).
그 결과 **투수×상황 클래스가 4전 4패로 닫혔다.** 판정 도구는 여지가 아니라 **지속성**이다(§7 마지막 항).
→ 판별식 3문: ① 정보가 느는가 ② 행마다 변하는가 ③ 해마다 이어지는가.
...
| 추론이 `KeyError: 's0_*'` | 옛 run 위에 얹는데 그 사이 `ANCHOR_SPECS`가 바뀌어 `anchor.csv` 스키마 불일치 | 목록을 그 run 구성으로 되돌리고 `anchor.csv` 재생성 — **값이 같은지 assert**할 것. `verify_submit.py`가 아니었으면 제출 1회 차감 |
