...
def engineer(df, global_mean, m=30, extra_fe=False, rate_means=None):
    ...
    if extra_fe:
        ...
        # # 11) Fatigue 피처 / 현재 타자 상대 투구 수(예측치, foul도 있기 때문에)
        # d["balls_for_this_batter"] = d["strikes_before"] + d["balls_before"]

        # # 12) Claude 추천
        # saprr = d["smoothed_asof_pitcher_reverse_rate"]
        # sapmr = d["smoothed_asof_pitcher_middle_rate"]
        # sabmr = d["smoothed_asof_batter_middle_rate"]
        # d["platoon_x_count"] = d["platoon_advantage"] * d["count_advantage"]
        
        # d["miss_tendency"] = saprr / (saprr + sapmr + 1e-12)
        # d["score_diff_abs"] = abs(d["score_diff_pitcher_team"])

        # d["p_matchup_middle"] = (sapmr * sabmr) / (sapmr * sabmr + (1 - sapmr) * (1 - sabmr) + 1e-12)

        # d["form_trend_5_1_middle"] = df["asof_pitcher_prev1_game_middle_rate"] - df["asof_pitcher_prev5_game_middle_rate"]
