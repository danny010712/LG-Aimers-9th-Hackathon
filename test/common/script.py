...
def main():
    ...
    fe = engineer(test.drop(columns=[ID]), meta["global_mean"],
                 ...
                 kitchen_sink=meta.get("kitchen_sink", False))
    ...
    main_baseline = None
    if lb.get("enabled"):
        ta = lb.get("team_adjustment")
        # test는 raw test.csv에서 season/game_type(및 팀 보정용 team_col)을
        # 봐야 한다 (fe/X는 문자열 캐스팅 등으로 원본과 dtype이 달라질 수
        # 있으므로 원본 test에서 가져옴).
        cols = ["season"] + (["game_type"] if "game_type" in lb["group_cols"] else [])
        if ta:
            cols = list(dict.fromkeys(cols + [ta["team_col"]]))
        raw = test[cols].copy()
        ...
        # 조항 참고).
        if ta:
            # 팀 단위 baseline 세분화(R 전용) — precheck_team_baseline.py로
            # 확인된 대로 R에만 팀 편차를 추가로 얹는다. F는 아래 team_mask가
            # False라 assign_team_adjusted_baseline_logit 내부에서 season×
            # game_type 표만 쓰는 기존 경로와 완전히 동일하게 처리된다.
            team_table, team_group_cols, team_col = lr.team_table_from_json(ta)
            team_mask = (raw["game_type"] == "R").values
            main_baseline = lr.assign_team_adjusted_baseline_logit(
                raw, table, group_cols, team_table, team_col, team_mask,
                held_out_season=None, season_override=lb.get("test_override"))
        else:
    ...
    if off:
        ...
        fe_aux = engineer(test.drop(columns=[ID]), af["global_mean"],
                          ...
                          kitchen_sink=af.get("kitchen_sink", False))
