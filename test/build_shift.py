...
RUN = "017_shift_league"
BASE_RUN = "016_offset_league"
...
def predict_full(mdir, meta, X, ci, fake):
    ...
    if lb.get("enabled"):
        ...
        if ta:
            # fake는 train.csv에서 그대로 가져온 행이라 team_col(pitcher_team_id)
            # 등 모든 원본 컬럼을 이미 갖고 있다 — script.py처럼 따로 뽑을 필요 없음.
            team_table, team_group_cols, team_col = lr.team_table_from_json(ta)
            team_mask = (fake["game_type"] == "R").values
            main_baseline = lr.assign_team_adjusted_baseline_logit(
                fake, table, group_cols, team_table, team_col, team_mask,
